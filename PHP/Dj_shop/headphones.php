...
<div id="page-wrapper">
	...
	<div class="container">
		<h1 class="text-center">Słuchawki</h1><hr>
		<?php 			
			...
			{
				$result = $db->query("SELECT id, name, price, image FROM products WHERE promotion=0 AND id_categories=4 ORDER BY id DESC ");
				if($result->results())
	...
	<div class="container">
		...
		<?php 
			if (hasPerm ([2], $user->data()->id))
			{
				$result = $db->query("SELECT id, name, price, image, price_after_discount FROM products WHERE promotion=1 AND id_categories=4 ORDER BY id DESC ");
				if($result->results())
				{
